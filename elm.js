...
var $author$project$Main$Model = F5(
	function (key, url, content, letter, fromDir) {
		return {content: content, fromDir: fromDir, key: key, letter: letter, url: url};
	});
...
var $author$project$Main$fetchArticle = function (letter) {
	return ((_Utils_cmp(
		...
		letter,
		_Utils_chr('z')) < 1)) ? $elm$http$Http$get(
		{
			expect: $elm$http$Http$expectString($author$project$Main$GotArticle),
			url: '/articles/' + $elm$core$String$fromChar(letter)
		}) : $elm$core$Platform$Cmd$none;
};
				...
				if (result.$ === 'Ok') {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								content: 'There was an error retrieving the content corresponding to the letter ' + $elm$core$String$fromChar(model.letter)
							}),
						$elm$core$Platform$Cmd$none);
				}
			case 'PressedLetter':
...
var $author$project$Main$viewArticle = function (model) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
				$elm$html$Html$Attributes$class('article'),
				$elm$core$String$isEmpty(model.content) ? $elm$html$Html$Attributes$class('') : $elm$html$Html$Attributes$class('w3-animate-' + model.fromDir)
			]),
		_List_fromArray(
			[
				$author$project$Main$toMarkdown(model.content)
			]));
};
	...
	return {
		body: _List_fromArray(
			[
				...
				(!$elm$core$Char$isAlpha(model.letter)) ? $author$project$Main$viewInfo : $author$project$Main$viewArticle(model),
