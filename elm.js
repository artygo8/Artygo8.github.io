...
{
	if (region.K.B === region.R.B)
	{
		return 'on line ' + region.K.B;
	}
	return 'on lines ' + region.K.B + ' through ' + region.R.B;
}
...
var _Http_toTask = F3(function(router, toTask, request)
{
	return _Scheduler_binding(function(callback)
	{
		...
		try {
			xhr.open(request.aJ, request.M, true);
		} catch (e) {
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {T: fragment, X: host, aa: path, ac: port_, af: protocol, ag: query};
	});
...
var $author$project$Main$Model = F4(
	function (key, url, content, letter) {
		return {m: content, Y: key, e: letter, M: url};
	});
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{ax: false, az: r.az, aE: r.aE, V: r.V, aJ: r.aJ, aV: r.aV, ar: r.ar, M: r.M}));
};
var $elm$http$Http$get = function (r) {
	return $elm$http$Http$request(
		{az: $elm$http$Http$emptyBody, aE: r.aE, V: _List_Nil, aJ: 'GET', aV: $elm$core$Maybe$Nothing, ar: $elm$core$Maybe$Nothing, M: r.M});
};
var $author$project$Main$fetchArticle = function (letter) {
	return ((letter >= 'a') && (letter <= 'z')) ? $elm$http$Http$get(
		{
			aE: $elm$http$Http$expectString($author$project$Main$GotArticle),
			M: '/articles/' + $elm$core$String$fromChar(letter)
		}) : $elm$core$Platform$Cmd$none;
};
var $elm$browser$Browser$Navigation$load = _Browser_load;
var $elm$core$Char$fromCode = _Char_fromCode;
var $author$project$Main$nextLetter = function (letter) {
	return (!$elm$core$Char$isAlpha(letter)) ? 'a' : ((letter === 'z') ? ' ' : $elm$core$Char$fromCode(
		$elm$core$Char$toCode(letter) + 1));
};
var $author$project$Main$previousLetter = function (letter) {
	return (!$elm$core$Char$isAlpha(letter)) ? 'z' : ((letter === 'a') ? ' ' : $elm$core$Char$fromCode(
		$elm$core$Char$toCode(letter) - 1));
};
		...
		switch (msg.$) {
				...
				if (!direction) {
					return _Utils_Tuple2(
						_Utils_update(
							...
							{
								m: '',
								e: $author$project$Main$nextLetter(model.e)
							}),
						$author$project$Main$fetchArticle(
							$author$project$Main$nextLetter(model.e)));
				} else {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								m: '',
								e: $author$project$Main$previousLetter(model.e)
							}),
						$author$project$Main$fetchArticle(
							$author$project$Main$previousLetter(model.e)));
				}
			case 4:
				var character = msg.a;
				return _Utils_Tuple2(
			...
			case 5:
				...
				return (s === 'ArrowLeft') ? _Utils_Tuple2(
					...
					$author$project$Main$fetchArticle(
						$author$project$Main$previousLetter(model.e))) : ((s === 'ArrowRight') ? _Utils_Tuple2(
					_Utils_update(
						model,
						{
							m: '',
					...
					$author$project$Main$fetchArticle(
						$author$project$Main$nextLetter(model.e))) : _Utils_Tuple2(model, $elm$core$Platform$Cmd$none));
			default:
				var result = msg.a;
				if (!result.$) {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								m: 'There was an error retrieving the content corresponding to the letter ' + $elm$core$String$fromChar(model.e)
							}),
						$elm$core$Platform$Cmd$none);
				}
		}
...
var $author$project$Main$arrow = function (dir) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
				$elm$html$Html$Attributes$class('w3-center arrow-button')
			]),
		_List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						...
						$author$project$Main$ChangePage(dir))
					]),
				_List_fromArray(
					[
						A2(
				...
				[
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$Attributes$class('fa fa-file-text w3-margin-right')
								]),
							_List_Nil),
							$elm$html$Html$text('click for résumé')
						]))
				]))
		]));
...
var $author$project$Main$pageFooter = A2(
	...
	_List_fromArray(
		[
			$elm$html$Html$Attributes$class('w3-center w3-padding-32')
		]),
	_List_fromArray(
		[
			A2(
			...
			_List_fromArray(
				[
					$elm$html$Html$Attributes$class('w3-xlarge w3-section text-decoration-none')
				]),
			_List_fromArray(
				[
...
var $author$project$Main$main = $elm$browser$Browser$application(
	{aI: $author$project$Main$init, aL: $author$project$Main$UrlChanged, aM: $author$project$Main$LinkClicked, aT: $author$project$Main$subscriptions, aX: $author$project$Main$update, aY: $author$project$Main$view});
